CREATE TYPE "public"."clinic_member_role" AS ENUM('owner', 'dietitian', 'assistant');--> statement-breakpoint
...
ALTER TABLE "clinic_members" ADD CONSTRAINT "clinic_members_invited_by_users_id_fk" FOREIGN KEY ("invited_by") REFERENCES "public"."users"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
CREATE UNIQUE INDEX "clinic_members_clinic_id_user_id_idx" ON "clinic_members" USING btree ("clinic_id","user_id");
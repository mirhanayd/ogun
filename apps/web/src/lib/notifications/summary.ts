...
export function isStaleMeasurementClient(client: ClientMeasurementRecencyInput, now: Date = new Date()): boolean {
...
export function packageExpiryState(expiresAt: Date, now: Date = new Date()): 'süresi_doldu' | 'yaklaşıyor' {
...
export interface NotificationFeedInput {
  todayAppointmentsCount: number
  noShowClients: { clientId: string; clientFirstName: string; clientLastName: string; startsAt: Date }[]
...
export function buildNotificationFeed(input: NotificationFeedInput, now: Date = new Date()): NotificationFeed {
  const staleMeasurementClients = input.clientsWithLastMeasurement.filter((c) => isStaleMeasurementClient(c, now))

...
export function SettingsSubpage({ title, children, notice }: { title: string; children: ReactNode; notice?: string | null }) {
  return <div className="flex flex-col gap-4 pb-8" data-settings-subpage={title}>
    <NavigationLink href="/ayarlar" className="w-fit text-sm text-primary hover:underline">← Ayarlara dön</NavigationLink>
...
export function TeamSettingsView({ clinicName, ...manager }: ComponentProps<typeof TeamManager> & { clinicName: string }) {
  return (
...
export function ReminderSettingsView({ template, onSave, disabled, sweepControl }: { template?: string | null; onSave: ComponentProps<typeof SmsTemplateForm>['onSave']; disabled?: boolean; sweepControl: ReactNode }) {
  return (
    <div className="flex flex-col gap-6 pb-8">
      <header className="border-b border-border/70 pb-6">
        <div className="flex items-center gap-2 text-xs font-semibold tracking-[0.14em] text-primary uppercase">
...
export function SharingSettingsView({ template, onSave, disabled }: { template?: string | null; onSave: ComponentProps<typeof WhatsappTemplateForm>['onSave']; disabled?: boolean }) {
  return (
...
export interface SettingsAuditLog { id: string; createdAt: string | Date; action: AuditAction; entityType: string; entityId: string | null; ipAddress: string | null }
const ACTION_LABELS_TR: Record<AuditAction, string> = { create: 'Oluşturma', read: 'Görüntüleme', update: 'Güncelleme', delete: 'Silme', export: 'Dışa aktarma' }
export function SecuritySettingsView({ retentionDays, recentLogs, onSave, disabled }: { retentionDays?: number | null; recentLogs: SettingsAuditLog[]; onSave: ComponentProps<typeof DataRetentionForm>['onSave']; disabled?: boolean }) {
  return (
    <div className="flex flex-col gap-4">
      <Card>
        <CardHeader>
        ...
        <CardContent>
          <DataRetentionForm defaultValues={{ dataRetentionDays: retentionDays ?? 3650 }} onSave={onSave} disabled={disabled} />
        </CardContent>
      </Card>
...
export function SubscriptionSettingsView() { return <Card><CardHeader><CardTitle>Abonelik</CardTitle><CardDescription>Abonelik yönetimi şu anda kullanıma açık değil.</CardDescription></CardHeader><CardContent><Button asChild variant="outline"><NavigationLink href="/ayarlar">Klinik ayarlarına dön</NavigationLink></Button></CardContent></Card> }

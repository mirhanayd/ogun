...
export async function MeasurementsTab({ clientId }: { clientId: string }) {
  ...
  return <MeasurementsView measurements={measurements} activeGoals={goals} weightGoal={weightGoal ? { targetValue: Number(weightGoal.targetValue) } : null} onSaveMeasurement={createMeasurementAction.bind(null, clientId)} onCreateGoal={createGoalAction.bind(null, clientId)} onAchieveGoal={onAchieveGoal} />
}

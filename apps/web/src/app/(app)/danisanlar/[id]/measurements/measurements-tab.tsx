...
export async function MeasurementsTab({ clientId }: { clientId: string }) {
  const [rows, weightGoal, bodyFatGoal, circumferenceGoal] = await Promise.all([
  ...
  const measurements: ChartMeasurement[] = rows.map((row) => ({
    id: row.id,
  ...
  const goals: ActiveGoalRow[] = [weightGoal, bodyFatGoal, circumferenceGoal]
  ...
  async function onAchieveGoal(goalId: string) {
    'use server'
    return achieveGoalAction(goalId, clientId)
  }
  return (
    <MeasurementsView
      measurements={measurements}
      activeGoals={goals}
      weightGoal={weightGoal ? { targetValue: Number(weightGoal.targetValue) } : null}
      onSaveMeasurement={createMeasurementAction.bind(null, clientId)}
      onCreateGoal={createGoalAction.bind(null, clientId)}

      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border/80 pb-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-muted-foreground mt-0.5">
            Aktif olarak size atanmış veya talep ettiğiniz{' '}
            <strong className="text-foreground">{total}</strong> klinik aday bulunuyor.
          </p>
        </div>
      ...
      {tasks.length === 0 ? (
        <div className="rounded-xl border border-dashed border-border p-12 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground max-w-md mx-auto">
            Üzerinizde bekleyen aktif bir inceleme bulunmuyor. Yeni görevler Ogun Operasyon ekibi
            tarafından atanır.
          </p>
          {session.profile.professionalRole === 'clinical_admin' ? (
            <div className="mt-6">

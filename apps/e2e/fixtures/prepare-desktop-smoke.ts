...
async function main() {
  const sourceUrl = process.env.CLINICAL_SOURCE_DATABASE_URL
  if (!sourceUrl) throw new Error('CLINICAL_SOURCE_DATABASE_URL must be explicitly set')
  const targetUrl = process.env.DESKTOP_SMOKE_DATABASE_URL
  assertLocalDatabaseTarget(targetUrl, 'Desktop smoke fixture writes')
  const targetLocation = new URL(targetUrl!)
  if (
    !['localhost', '127.0.0.1'].includes(targetLocation.hostname) ||
    targetLocation.pathname !== '/ogun_patch_035'
  ) {
  ...
  try {
    for (const table of [
      ...
      if (existing[0]?.count > 0) {
        console.log(`${table}: already copied (${existing[0]?.count})`)
        continue
      }
    ...
    const foodRows =
      await source`select * from foods where created_by is null and name_tr ilike '%a%' and search_text like '%e%' order by id limit 25`
    ...
    const sourceMap = new Map(
      sourceDefinitions.map((row) => [
        row.id,
        targetDefinitions.find((item) => item.code === row.code)?.id,
      ]),
    )
    ...
    for (const food of foodRows) {
      ...
      const values =
        await source`select fn.*, n.code from food_nutrients fn join nutrients n on n.id=fn.nutrient_id where fn.food_id=${food.id}`
      const mapped = values
        .map(({ code, ...value }) => ({
          ...value,
          nutrient_id: nutrientDefinitions.find((n) => n.code === code)?.id,
          source_id: sourceMap.get(value.source_id),
        }))
        .filter((value) => value.nutrient_id && value.source_id)
      if (mapped.length)
        await target`insert into food_nutrients select * from json_populate_recordset(null::food_nutrients, ${JSON.stringify(mapped)}::text::json) on conflict do nothing`
    }
    ...
    for (const role of ['owner', 'dietitian', 'assistant'] as const) {
      const [user] = await db
        .insert(users)
        .values({
          name: `Release Test ${role}`,
          email: `release-${role}-${suffix}@ogun.test`,
          emailVerified: true,
        })
        .returning()
    ...
    const [clinic] = await db
      .insert(clinics)
      .values({
        name: 'Öğün Sürüm Doğrulama Kliniği',
        ...
        logoUrl:
          ...
          Buffer.from(
            '<svg xmlns="http://www.w3.org/2000/svg" width="80" height="80"><rect width="80" height="80" rx="20" fill="#6D4AFF"/><text x="40" y="55" text-anchor="middle" font-size="48" fill="white">Ö</text></svg>',
          ).toString('base64'),
      })
      .returning()
    ...
    for (const user of people)
      await db
        ...
        .values({ userId: user.id, clinicId: clinic.id, role: user.role, joinedAt: new Date() })
    ...
    const [condition] =
      await target`select id, name_tr from conditions where is_active and search_text like '%diyabet%' order by length(name_tr), id limit 1`
    const [product] =
      await target`select id, name from medication_products where is_selectable and name ilike 'PAROL%' order by length(name), id limit 1`
    const [substance] =
      await target`select id, name_tr from medication_substances where search_text like '%metfor%' order by length(name_tr), id limit 1`
    if (!condition || !product || !substance)
      throw new Error('Required canonical source entries not found')
    await db
      .insert(clientHealth)
      .values({ clientId: client.id, conditions: [condition.name_tr], medications: [product.name] })
    ...
    await db
      ...
      .values({
        clientId: client.id,
        measuredAt: new Date('2026-08-25T09:00:00Z'),
        weightKg: '77.2',
        source: 'manuel',
        recordedBy: owner.id,
      })
    await db
      ...
      .values({
        ...
        startsAt: new Date('2026-08-28T09:00:00Z'),
        endsAt: new Date('2026-08-28T09:30:00Z'),
        status: 'geldi',
        type: 'kontrol',
      })
    const output = resolve(
      import.meta.dirname,
      '../../desktop/src-tauri/target/release-smoke/fixture.json',
    )
    await mkdir(resolve(import.meta.dirname, '../../desktop/src-tauri/target/release-smoke'), {
      recursive: true,
    })
    await writeFile(
      output,
      JSON.stringify(
        {
          clinic,
          client,
          people: people.map(({ id, name, email, role }) => ({ id, name, email, role })),
          password,
          condition,
          product,
          substance,

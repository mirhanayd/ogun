...
test.describe('Klavye gezinmesi', () => {
  test('plan editörü Tab sırası: arama → miktar → sonraki kalem', async ({ page }) => {
    ...
    // Web and packaged desktop share the same non-modal topbar search.
    const search = page.getByRole('combobox', { name: 'Sayfa, ayar veya danışan ara' })
    await search.click()
    const results = page.locator('[data-command-panel]:visible')
    await expect(results).toHaveCount(1)
    await expect(page.getByRole('dialog')).toHaveCount(0)
    const triggerBounds = await page.locator('[data-command-trigger]:visible').boundingBox()
    const resultBounds = await results.boundingBox()
    expect(resultBounds!.y).toBeGreaterThanOrEqual(triggerBounds!.y + triggerBounds!.height)
    expect(Math.abs(resultBounds!.width - triggerBounds!.width)).toBeLessThan(2)
    await page.locator('main h1').click()
    await expect(results).toHaveCount(0)
    await page.keyboard.press('Control+k')
    await expect(results).toHaveCount(1)
    await expect(search).toBeFocused()
    await search.fill('Danışanlar')
    await expect(results.locator('[cmdk-item][aria-selected="true"]')).toContainText('Danışanlar')
    await search.press('ArrowDown')
    await expect(results.locator('[cmdk-item][aria-selected="true"]')).not.toContainText('Danışan listesi ve kayıtları')
    await search.press('ArrowUp')
    await expect(results.locator('[cmdk-item][aria-selected="true"]')).toContainText('Danışan listesi ve kayıtları')
    await search.press('Enter')
    await expect(page).toHaveURL(/\/danisanlar$/)
    await page.keyboard.press('Control+k')

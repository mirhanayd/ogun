  ...
  return (
    <div className="app-shell">
      <header className="topbar">
        <div><strong>Ogun Operasyon</strong></div>
        <div className="user-box">
          <div><strong>{ctx.user.name}</strong> · {ctx.staff.role}</div>
          <SignOutButton />
        </div>
      </header>
      <aside className="sidebar" aria-label="Ana navigasyon">
        <nav className="nav">
          {visible.map((item) =>
            item.href ? <Link href={item.href} key={item.label}>{item.label}</Link> : <span className="disabled" key={item.label}>{item.label} · Yakında</span>,
          )}
        </nav>
      </aside>
